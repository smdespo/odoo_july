...
export default function FuelLogForm({ vehicles, onSubmit, onCancel, isSubmitting }) {
  const {
    register,
    handleSubmit,
    formState: { errors },
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
      ...
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <input
            type="number"
            step="0.1"
            {...register("liters", { required: "Required", valueAsNumber: true, min: { value: 0.1, message: "Must be positive" } })}
      ...
      <div className="flex justify-end gap-2 pt-2">
        ...
        >
          {isSubmitting ? "Saving..." : "Log fuel entry"}
        </button>
      </div>
    </form>
  );
